...
<script type="text/javascript">
    $(function () {
        ...
        function save_tax_type_details() {
            ...
            var sno = document.getElementById('tax_sno').value;
            var btn_val = document.getElementById('btn_save').value;
            var data = { 'op': 'save_tax_type_details', 'taxtype': taxtype, 'description': desc, 'sno': sno, 'btn_val': btn_val };
            var s = function (msg) {
                if (msg) {
                    if (msg.length > 0) {
                        alert(msg);
        ...
        function filldetails(msg) {
            ...
            for (var i = 0; i < msg.length; i++) {
                ...
                results += '<td style="display:none" data-title="sno" class="4">' + msg[i].sno + '</td>';
            }
        ...
        function fillglgroup(msg)
        {
            var compiledList = [];
            for (var i = 0; i < msg.length; i++) {
                var code = msg[i].groupshortdesc;
                compiledList.push(code);
            }

            $('#txt_gl_desc').autocomplete({
                source: compiledList,
                change: calTotal,
                autoFocus: true
            });
        ...
        //                $(this).closest('tr').find('#txt_desc1').val(shortdes[i].groupcode);
        ...
        //    }
        //}
        function calTotal(txt_gl_desc) {
            var code = document.getElementById('txt_gl_desc').value;
            for (var i = 0; i < shortdes.length; i++) {
                if (code == shortdes[i].groupshortdesc) {
                    document.getElementById('txt_gl_code').value = shortdes[i].groupcode;
                    document.getElementById('txt_gl_code_sno').value = shortdes[i].sno;
        ...
        function save_tax_percentage_details() {
            ...
            //    }
            //});
            var group_code = document.getElementById('txt_gl_code_sno').value;
            if (group_code == "") {
            ...
            var taxcode = document.getElementById('txt_taxid').value;
            ...
            var data = { 'op': 'save_tax_percent_details', 'sno': sno, 'groupcode': group_code, 'taxcode': taxcode, 'rangefrom': rangefrom, 'rangeto': rangeto, 'percent': percent, 'effFrom': effFrom, 'effTo': effTo, 'status': status, 'btn_val': btn_val };
            var s = function (msg) {
                if (msg) {
                    if (msg.length > 0) {
                        alert(msg);
        ...
        function fill_foreground_tbl(msg) {
            ...
            results += '<thead><tr><th scope="col"></th><th scope="col">GROUP LEDGER</th><th scope="col">TAXCODE</th><th scope="col">RANGE_FROM</th><th scope="col">RANGE_TO</th><th scope="col">PERCENTAGE</th><th scope="col">EFFECTIVE_FROM</th><th scope="col">EFFECTIVE_TO</th><th scope="col">STATUS</th></tr></thead></tbody>';
            for (var i = 0; i < msg.length; i++) {
                results += '<tr><td><input id="btn_poplate" type="button"  onclick="update(this)" name="Edit" class="btn btn-primary" value="Edit" /></td>';
                results += '<td data-title="GROUP LEDGER" class="9" >' + msg[i].groupcodedesc + '</td>';
                results += '<td style="display:none;" data-title="GROUP LEDGER" class="10" >' + msg[i].groupcode + '</td>';
                results += '<td style="display:none;" data-title="GROUP LEDGER" class="11" >' + msg[i].groupcodesno + '</td>';
                results += '<td data-title="TAXCODE" class="1" >' + msg[i].taxcodedesc + '</td>';
                results += '<td style="display:none;" data-title="TAXCODE" class="12" >' + msg[i].taxcode + '</td>';
                results += '<td style="display:none;" data-title="TAXCODE" class="13" >' + msg[i].taxcodesno + '</td>';
        ...
        function update(thisid) {
            var groupcodedesc = $(thisid).parent().parent().children('.9').html();
            var groupcode = $(thisid).parent().parent().children('.10').html();
            var groupcodesno = $(thisid).parent().parent().children('.11').html();
            var taxcodedesc = $(thisid).parent().parent().children('.1').html();
            var taxcode = $(thisid).parent().parent().children('.12').html();
            var taxcodesno = $(thisid).parent().parent().children('.13').html();
            ...
            document.getElementById('slct_taxcode').value = taxcodedesc;
            document.getElementById('txt_taxid').value = taxcodesno;
            document.getElementById('txt_taxcode').value = taxcode;
            document.getElementById('txt_gl_code_sno').value = groupcodesno;
            document.getElementById('txt_gl_code').value = groupcode;
            document.getElementById('txt_gl_desc').value = groupcodedesc;
        ...
        function clear_tax_percentage_Details() {
            ...
            document.getElementById('txt_gl_code_sno').value = "";
            document.getElementById('txt_gl_code').value = "";
            document.getElementById('txt_taxcode').value = "";
            document.getElementById('slct_taxcode').value = "";
            document.getElementById('txt_taxid').value = "";
                     ...
                     <div id='showlogs2' style="display: none;">
                        <table align="center" style="width: 60%;">
                            ...
                            <tr>
                                <td>
                                    <label>
                                        Group Ledger Code</label>
                                    <%--<span style="color: red;">*</span>--%>
                                </td>
                                <td>
                                    <input id="txt_gl_desc" class="form-control" placeholder="Enter Group Ledger"></input>
                                    <input id="txt_gl_code_sno" type="hidden"></input>
                                   
                                </td>
                                <td style="width: 5px">
                                </td>
                                <td style="height: 40px;">
                                    <input id="txt_gl_code" class="form-control" type="text"  placeholder="Group Ledger Code" name="gl_code" readonly="readonly" />
                                </td>
                            </tr>
                            <tr>
                                <td>
                                ...
                                <td style="height: 40px;">
                                    <input id="txt_taxcode" class="form-control" type="text"  placeholder="Tax Type Description" name="taxcode" readonly="readonly" />
                                </td>
                            </tr>
                            <tr>
                                <td style="height: 40px;">
